...
### Refresh lock
При конкурирующих запросах используется lock, чтобы не обновлять токен параллельно:
- ключ строится по `CacheAwareInterface::getCacheKey()` или классу auth
- TTL берётся из `ClientConfig::timeout` (минимум 5s, максимум 30s)
- ожидание — через `SleeperInterface` (см. observability)

## Кеш для токенов
Если authenticator реализует `CacheAwareInterface`, SDK передаст PSR‑16 store
из `ClientConfig::cache` или `ClientConfig::cacheConfig->store`.
