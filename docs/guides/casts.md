...
`JsonCast::hydrate()` автоматически сохраняет большие целочисленные JSON-литералы
строками. Его permissive-поведение для malformed JSON не меняется; serialize остаётся
строгим. Общая политика чисел описана в [сериализации](serialization.md#большие-целые-в-ответах).
Пользовательские casts сохраняют приоритет и отвечают за точность своего преобразования.
