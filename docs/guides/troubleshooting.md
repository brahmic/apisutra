...
**Решение:** `RateLimitConfig::store` позволяет обмениваться счётчиком, но get/set
не гарантируют атомарную квоту между процессами. Без store счётчик принадлежит
экземпляру клиента. См. [границы механизма](client-config/rate-limit.md#ключ-и-store).
