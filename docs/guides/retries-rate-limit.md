...
- `totalTimeoutMs` — проверка прошедшего времени перед очередной попыткой; пока не
  ограничивает длительность HTTP-вызова и всех ожиданий. Полноценный deadline не реализован.
