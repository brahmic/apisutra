...
| 1.2 | Базовые классы определены | BaseClient, BaseRequest, BaseResource, BaseDto, BaseResponseDto. Централизуют дефолты и правила. BaseDto/BaseResponseDto — рекомендуемая точка binding профилей в атрибутной модели; plain DTO с внешними правилами не требуют этих базовых классов. | [provider-methodology.md#2-базовые-классы](provider-methodology.md#2-базовые-классы-вместо-копипаста) | |
...
| № | Пункт | Пояснение / подсказка | Ссылка на детали | Мои заметки |
|---|-------|------------------------|------------------|-------------|
| 2.1 | Типы и свойства DTO | readonly, типизированные свойства. Глубина определяется контрактом API и нужными потребителю данными; для сложной вложенности проверьте читаемость и группировку типов. Типы дат: DateTimeImmutable/Carbon. Для обычных scalar-полей в Legacy действует safe auto-cast; Strict внешнего набора требует точных типов (с допустимым int → float). Если DTO строится по inheritance-модели, constructor-first остаётся основным контрактом, но конечный DTO теперь может добавлять public hydrated properties вне constructor chain. Если провайдер системно присылает `''` вместо отсутствия значения, продумайте `EmptyStringAsNull`, `emptyStringBehavior` профиля или `RulePolicy::emptyString`. | [dto.md](dto.md), [glossary/dto.md](../glossary/dto.md), [casts.md](casts.md) | |
...
| 2.6 | Naming strategy | Для DTO hydration naming fallback задайте `DtoHydrationProfile` или `RulePolicy::naming` внешнего набора, для body DTO naming policy — через `DtoSerializationProfile`. Для request/query/header/path fallback по-прежнему используется клиентский config. | [naming-strategy.md](naming-strategy.md), [client-config/serialization.md](client-config/serialization.md) | |
| 2.7 | #[DefaultValue] | Когда поле может отсутствовать — `#[DefaultValue(value: X)]` (по умолчанию Missing) либо `DefaultSpec` внешних правил. Provider создаёт динамические значения. Автодефолт typed collection не заменяет `required()`. | [attributes/data-transfer.md](attributes/data-transfer.md) | |
...
| 3.3a | Правила DTO и сериализация | Входящие правила: `DtoHydrationProfile` для атрибутной модели либо внешний `HydrationRules` для plain DTO. DX: `DtoSerializationProfile`. Не совмещайте `DtoRules` с входящим профилем одного класса. Для wire body по умолчанию используйте safe transport policy; если нужно, явно задайте `wireBodySerializationPolicy`. Для query/header/path задавайте отдельные request-level rules в `ClientConfig`. | [hydration-rules.md](hydration-rules.md), [client-config/serialization.md](client-config/serialization.md#dtoserializationprofile), [serialization.md](serialization.md#enum-сериализация) | |
| 3.3b | Неизвестные поля и исходящий payload | При необходимости объявите receiver `_extra`; клиент с набором исключает его и у вручную созданных DTO. Проверьте ограничения cast/opaque-обёрток. `toArray()` и `DTO::from()` набор не наследуют; standalone-гидратация — через `forRules()`. | [hydration-rules.md](hydration-rules.md#receiver-в-исходящих-запросах) | |
...
| 5.10 | Provider Async Await | Optional sync/async через один метод — `#[ContinuationResult]`, `ContinuationMode`, `await()/awaitAs()`, `ContinuationModeApplicatorInterface`. Один request на бизнес-действие. Объявите критерий Ready/Pending/Failed: stateResolver или непустой unwrap. Ошибки Ready-payload завершают ожидание; проверьте миграцию и обработку `ContinuationAwaitException`. | [provider-async-await.md](provider-async-await.md) | |
