...
## Внешний набор и receiver

Если клиенту передан HydrationRules с `extras()`, сериализатор исключает receiver
из запросов по классу DTO, включая вручную созданные модели и вложенные значения.
DX `toArray()` без набора сохраняет обычное свойство. Правила plain-представления,
запрет непрозрачных casts и прежние ограничения query описаны в
[справке внешних правил](hydration-rules.md#receiver-в-исходящих-запросах).
