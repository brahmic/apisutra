...
Абсолютный endpoint (`https://other.test/...`), network-path (`//host/...`), схемы,
управляющие символы, неэкранированные пробелы, обратный slash и сегменты `.`/`..`
отклоняются с `configuration_error` до авторизации и HTTP.
...
или конфиг. Поддержка полного внешнего или подписанного endpoint пока не предусмотрена;
этот механизм не задаёт политику переноса credentials между origin.
...
- Незаполненные path-параметры, пустые/dot-сегменты и абсолютные endpoint теперь
