...
режим без SDK-лимитов — в [руководстве транспорта](../execution/transport.md).
...
[Приоритеты, ручная проверка DTO и миграция](../client/validation.md#как-подключается-валидатор).
...
собирайте backend в ServiceProvider. [Готовый рецепт](redis.md#laravel).
