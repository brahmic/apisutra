...
Имена классов относятся к `Brahmic\ApiSutra\Attributes\Response`.
...
| Returns | CLASS | `Returns(string $response, ?string $unwrap = null, ?string $type = null)` |
...
без DTO сохраняет [свой контракт](../results/handles.md#успешный-ответ-без-dto).
...
Полный контракт, пример resolver и миграция — в [Provider Async Await](../../guides/recipes/continuation.md).
...
[контракт ответа](../results/handles.md#успешный-ответ-без-dto).
