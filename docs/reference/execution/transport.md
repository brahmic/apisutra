# Транспорт, promise и ограничения I/O

## Promise API и фактическое выполнение

`sendAsync()` возвращает ResultHandle, а rawAsync()/resolvedAsync() дают promise.
Штатный Pipeline.executeAsync() выполняет синхронный путь и оборачивает его результат.
HttpTransport.sendAsync() также выполняет синхронный HTTP. Поэтому наличие promise
не гарантирует non-blocking, fire-and-forget или параллельность сетевых запросов.

Batch/pool управляют задачами согласно [своему контракту](batch-pool.md); реальную
конкурентность подтверждают выбранный исполнитель и транспорт. Отложенная операция
провайдера — отдельный [контракт готовности](continuation-state.md).
...
исходный query и повторяющиеся параметры по [контракту URI](../serialization/uri-query.md#uri-и-path).
...
Контракт владения и миграция — в [гайде файлов](../../guides/recipes/files.md). Сам пакет не получает
...
Миграция и пользовательский API: [внешние URL](../serialization/uri-query.md).
