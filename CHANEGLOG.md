...
## v0.5.0-alpha.1 — 2026-09-15

- **Миграция API кеша:** store и параметры объединены в единственном
  `ClientConfig::cacheConfig`. Аргумент/поле `cacheStore` удалён, добавлен
  `CacheConfig::store`. Новый `CacheConfig::with()` изменяет только переданные поля,
  сохраняя остальные значения и ссылки на зависимости без IO или clone.
- `ClientConfig::with(cacheConfig: ...)` заменяет блок целиком, без наследования
  прежнего store. null удаляет блок и явный locks; частичный `with(store: null)`
  отключает только store. Пустая и несвязанная копия клиента сохраняет блок.
  HTTP/auth используют общий backend; ключи и сохранённые записи не меняются.
  [Переход с v0.4.0-alpha.1](docs/migration/v0.5.0-alpha.1.md).
- Обновлены справочник и исполняемый обзор конфигурирования клиента. Добавлены
  [файловые примеры](docs/example/files/README.md) и сценарии сериализации в
  [обзоре DTO](docs/guides/dto/showcase.md).
...
- **Миграция конфигурации кеша:** единственный backend теперь задаётся через
  ...
  Старые `cache:` и `CacheConfig(store: ...)` удалены. `with()` больше не теряет store
...
- Добавлены [внешние правила гидратации](docs/guides/dto/plain-models.md): неизменяемые
  ...
  Рекомендуемое имя receiver в документации — `_extra`; имя остаётся настраиваемым,
  непрочитанные ключи `extra` и `_extra` сохраняются внутри остатка. Дополнены миграция,
  методология SDK и справочник новых API.
...
- CI проверяет PHP 8.4/8.5, locked/lowest/latest зависимости, Laravel 12,
  Redis 7.0/8.2, качество кода и установку архивов без dev-зависимостей.
- 1664 уникальных теста: 1647 основных и 17 Redis-сценариев. Бейдж объединяет
  отчёты, не дублируя тесты между окружениями.

### 2026-09-13 — внешний дедлайн, форматы ответа и безопасность повторов

- Добавлен необязательный `ExecutionDeadline::afterMs()` для общего срока нескольких
  вызовов: `withDeadline()` применяет его, `withoutDeadline()` снимает только внешний
  предел. HTTP, delay, квоты, backoff, Retry-After и auth refresh используют остаток
  одного бюджета. Без опции отдельные вызовы сохраняют прежнее поведение.
- Исправлена классификация Guzzle/cURL 55/56: `connection_failed` вместо
  `invalid_request`. Добавлена консервативная диагностика `not_sent/unknown`,
  сохраняющая неопределённость предыдущих попыток. Она не разрешает повтор POST.
- Добавлены `#[RawResponse]` и `withRawResponse()` для строки тела без декодирования.
  Неизвестный явно указанный не-JSON Content-Type без DTO возвращается строкой,
  с DTO/пагинацией — явной ошибкой формата. Raw несовместим с DTO и потоковым download.
- Необязательный `RetrySafetyPolicyInterface` позволяет запросу оценивать безопасность
  по текущему response/exception, например разрешать POST только после 429.
  Явный `Retry::safe` сохраняет приоритет; ограничения тела, попыток и бюджета действуют.
- HTTP `RateLimitException::retryAfter` и встроенное ожидание используют общий разбор
  секунд/HTTP-date на одних часах. Неверное значение теперь null, корректный ноль
...
- Исправлено замещение общей квоты клиента лимитом операции: теперь обе действуют
  перед каждой HTTP-попыткой. `includeClientQuota` управляет участием; атрибут
  может содержать только этот флаг. `withoutRateLimit()` отключает все квоты.
- Без настройки backend набор учитывается локально, с монотонными окнами. Добавлен
  публичный атомарный контракт и готовый необязательный backend Redis/phpredis.
  Wait/Throw и общий deadline обрабатываются одинаково для всех backend.
...
- Команда `composer test:redis` автоматически поднимает изолированный Docker Compose
  стенд и очищает его после проверки; установка phpredis в локальный PHP не нужна.
- Добавлены проверки независимыми workers, сбоев Redis и Laravel config:cache.
  Ядро не требует Redis/Laravel; дополнительные обязательные зависимости отсутствуют.
