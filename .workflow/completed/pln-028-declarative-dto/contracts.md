...
Для query receiver исключается на стадии сборки частей запроса. Ограничение URL builder
сохраняется: допустимы скаляры и плоские списки скаляров. Структурированный DTO или список
DTO после исключения receiver по-прежнему даёт `SerializationException` до HTTP.
Набор не вводит кодирование структур в query, а whole-value cast с receiver запрещён
правилом ниже. SDK формирует допустимые query-поля явной моделью запроса.
...
| Сочетание | Поведение |
| --- | --- |
| Класс с `extras()` реализует `DateTimeInterface`, `JsonSerializable` или `Stringable` либо имеет `toArray()` без `DtoInterface` | `ConfigurationException` при компиляции набора |
...
На 2026-09-15 все порции [реализованы и приняты](implementation.md), код `9e84782`.

| Порция | Разделы | Итог |
| --- | --- | --- |
| A: правила, strict и форма | 1–8, 12–13; внутренний учёт происхождения | Выполнена; 031 и 030 завершены |
| B: дополнительные поля | 9–10 | Выполнена; 031 завершён |
| C: sourcePath и безопасный лог | 11 | Выполнена |
