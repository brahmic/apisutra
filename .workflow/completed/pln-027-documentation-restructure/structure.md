...
Принятая раскладка для [плана 027](pln-027-readme.md), база `cddd036`.
Все пути дерева реализованы; проверка — в [отчёте](implementation.md). Старые страницы
перехода здесь не показаны; их адреса сохранены по [карте](content-map.md).
...
- `reference/client/configuration.md` — каталог **всех** полей ClientConfig:
  ...
  `with()` описан здесь; сборка и зависимости — в `client/construction.md`.
...
- `dto/field-rules.md` — builders HydrationRules/DtoRules/FieldRule, регистрация
  класса, конфликты атрибутов и компиляция. Приоритеты внешней RulePolicy находятся здесь; `dto/profiles.md` описывает
  атрибутный профиль, наследование и даты, ссылаясь на границу конфликта набора.
- `dto/scalars.md`, `defaults.md`, `shapes.md`, `variants.md` — соответственно
  ...
  поля описывается в `defaults.md`, подробные правила — у этих владельцев.
...
Базовый проект содержит 135 Markdown-страниц без переходов. Добавлены три страницы:
`reference/client/response-dto-catalog.md` отделяет производный каталог типов ответов
от статических provider catalogs; прежние 511 строк описывали две разные задачи.
Два README рядом с исполняемыми hydration-rules и continuation объясняют их запуск.
Итоговое дерево — 138 страниц без переходов. PHP-файлы примеров перечислены в их README.

ResultMetaExtractor относится к любому результату, поэтому его полный контракт
расположен в `reference/results/handles.md`, а не в разделе continuation.
Эти уточнения не меняют runtime API и зафиксированы в карте содержания.
