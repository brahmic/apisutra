...
База реализации 033 — `3ff9531`, после завершённого 032. Снимок готовности выше
остаётся историческим: его исходные SHA/path проверяются на соответствующем commit.
[Baseline реализации](implementation-baseline/commands.json),
[результаты](implementation/commands.json) и [матрица приёмки](../implementation.md)
не заменяют и не переписывают исходный issue.

Повторная проверка из корня, с новым каталогом для логов:

```bash
python3 .workflow/completed/pln-033-cache-config-copy/artifacts/verify-implementation.py --output /tmp/apisutra-033-recheck
```

Широкий PHPCS сохраняет известный ненулевой исход старых файлов; целевой новый
код чист. Для сравнения стиля с принятой базой служит
`compare-style.py --output /tmp/apisutra-033-style.json`; существующий файл не перезаписывается. Исторические capture
и readiness-скрипты используют старый API и запускаются на указанной в отчёте базе.
Исходный `--verify` сравнивает старое поведение; адаптированные наблюдения проверяются
отдельно по ручным ожиданиям четырёх изменений и неизменным AS-5 наблюдениям.
