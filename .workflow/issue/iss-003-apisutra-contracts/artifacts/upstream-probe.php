...
require __DIR__.'/fixtures/Store.php';
require __DIR__.'/fixtures/AppProvider.php';
require __DIR__.'/fixtures/ProbeRequest.php';
require __DIR__.'/fixtures/AuditClient.php';
require __DIR__.'/fixtures/AuditHttpClient.php';
require __DIR__.'/fixtures/ConstructorOwned.php';
require __DIR__.'/fixtures/OwnedEnvelope.php';
require __DIR__.'/fixtures/ReadOwned.php';
...
observe('fromLaravel reads only debug and environment automatically', function () {
    ...
    return ['calls' => $provider->calls, 'debug' => $config->debug, 'environment' => $config->environment->value, 'logger' => $config->logger, 'cache' => $config->cache, 'auth' => $config->auth];
});
...
observe('cache baseline', fn () => repeatGet($base->with(cache: new CacheConfig(store: new Store, ttl: 60))));
observe('with cacheConfig replacement', function () use ($base) {
    $old = new CacheConfig(store: new Store, ttl: 60);
    $replacement = new CacheConfig(store: new Store, ttl: 10);
    $config = $base->with(cache: $old)->with(cacheConfig: $replacement);
...
observe('with cache null leaves cacheConfig effective', function () use ($base) {
    $config = $base->with(cache: new CacheConfig(store: new Store, ttl: 60))->with(cache: null);

    return ['cacheIsNull' => $config->cache === null, 'cacheConfigIsNull' => $config->cacheConfig === null, 'execution' => repeatGet($config)];
});
observe('with both cache entries null disables', fn () => repeatGet($base->with(cache: new CacheConfig(store: new Store))->with(cache: null, cacheConfig: null)));
observe('with cache replacement works', function () use ($base) {
    $replacement = new CacheConfig(store: new Store, ttl: 10);
    $config = $base->with(cache: new CacheConfig(store: new Store, ttl: 60))->with(cache: $replacement);
...
observe('with unrelated option loses separately supplied cache store', function () use ($base) {
    $store = new Store;
    $config = $base->with(cache: $store, cacheConfig: new CacheConfig(ttl: 60));
    $before = repeatGet($config);
    $store->clear();
    $copy = $config->with(timeout: 7);

    return ['originalUsesStore' => $config->cache === $store, 'copyUsesStore' => $copy->cache === $store, 'before' => $before, 'after' => repeatGet($copy)];
});
observe('with no overrides also loses separately supplied cache store', function () use ($base) {
    ...
    return ['originalUsesStore' => $config->cache === $store, 'copyUsesStore' => $copy->cache === $store, 'execution' => repeatGet($copy)];
});
