...
use Brahmic\MaxSutra\Errors\ProtocolException;
use Brahmic\MaxSutra\Internal\ApiSutra\ApiSutraExecutor;
...
function client(RecordingHttpClient $http, ?ClientConfig $config = null): MaxApiClient
{
    $factory = new HttpFactory;

    return new MaxApiClient($config ?? new ClientConfig(
...
$http = new RecordingHttpClient([new Response(200, ['Content-Type' => 'application/json'], json_encode(['data' => $missing], JSON_THROW_ON_ERROR))]);
try {
    (new ApiSutraExecutor(client($http), 'fixture-token'))->execute(new ReadRecord);
    throw new RuntimeException('Expected the current MaxSutra executor to translate the failure.');
} catch (ProtocolException $error) {
    verify('P20', $error->fieldPath === '$', 'Current MaxSutra executor loses the structured hydration field path; adaptation is required.');
}
...
echo json_encode([
    'php' => PHP_VERSION,
    'apisutra_version' => InstalledVersions::getPrettyVersion('brahmic/apisutra'),
    'apisutra_reference' => InstalledVersions::getReference('brahmic/apisutra'),
