...
it('истёкший срок не читает кеш и не запускает auth или BeforeSend', function (): void {
    ...
    $cache = new SpyCache();
    $transport = new MockTransport();
    $client = new TestClient(new ClientConfig(baseUrl: 'https://fixture.test', cache: new CacheConfig(store: $cache), auth: new RefreshingAuthenticator()), $transport, $clock, $clock);
